...
- Not just plumbing: reimplement each stochastic step we want (start with
...
- **MC-style (drop lever 3):** run the sampler at global σ + native composite, don't compress
  ...
  [motion-context-comparison](motion-context-comparison.md)) — i.e. it re-introduces the exact bug
