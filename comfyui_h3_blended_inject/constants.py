...
def row_center_times(row_idx: int) -> tuple[float, ...]:
    ...
    """
    raise NotImplementedError("row_center_times: compute from FRAME_PER_TOKEN grid")
...
def row_frame_count(row_idx: int) -> int:
    ...
    ValueError
        If ``row_idx`` is negative.
    """
    raise NotImplementedError("row_frame_count: FRAME_PER_TOKEN[min(row_idx, 1)]")
...
def frame_to_row(frame_idx: int) -> int:
    ...
    """
    raise NotImplementedError("frame_to_row: 0 if frame_idx == 0 else 1 + (frame_idx - 1) // 4")
...
def total_rows(n_frames: int) -> int:
    ...
    """
    raise NotImplementedError("total_rows: 1 + ceil((n_frames - 1) / 4)")
...
def video_row_to_audio_tick(row_idx: int) -> int:
    ...
    ValueError
        If ``row_idx`` is negative.
    """
    raise NotImplementedError("video_row_to_audio_tick: compute from AUDIO_HZ / FPS ratio")
...
def audio_ticks_for_rows(n_rows: int) -> int:
    ...
    """
    raise NotImplementedError("audio_ticks_for_rows: derive from AUDIO_HZ and FPS via n_rows")
...
def time_shift_sigma(sigma: float) -> float:
    ...
    """
    raise NotImplementedError("time_shift_sigma: apply H3 audio shift formula from model.py")
